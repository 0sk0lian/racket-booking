                                      ...
                                      <div key={`${membership.club_id}-${membership.status}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted)', padding: '4px 0' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 4 }}>
                                          {membership.status === 'pending' && (
                                            <>
                                              <button onClick={() => updateMembership(membership.id, 'approved')} disabled={editBusy} style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 600, border: '1px solid #a7f3d0', background: '#ecfdf5', color: '#059669', cursor: 'pointer', fontFamily: 'inherit' }}>Godkänn</button>
                                              <button onClick={() => updateMembership(membership.id, 'rejected')} disabled={editBusy} style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 600, border: '1px solid #fecaca', background: '#fef2f2', color: '#dc2626', cursor: 'pointer', fontFamily: 'inherit' }}>Avslå</button>
                                            </>
                                          )}
