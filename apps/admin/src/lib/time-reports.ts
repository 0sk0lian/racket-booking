...
export type SyncTimeReportsResult = {
  created: number;
  totalHours: number;
  totalPay: number;
  reports: SyncedTimeReport[];
};
...
export function parseRateMap(value: unknown): Record<string, number> {
...
export async function syncScheduleForTrainer({
  userId,
  clubId,
  date,
}: {
  ...
  if (courtIds.length === 0) {
    return { created: 0, totalHours: 0, totalPay: 0, reports: [] };
  }
  ...
  const previewRows: Array<{ bookingId: string; sessionTitle: string; category: string; hours: number; rate: number; pay: number }> = [];
  ...
  for (const booking of bookings) {
    ...
    const title = booking.booking_type === 'event'
      ...
      : `Träning ${booking.time_slot_start.slice(11, 16)}`;
    ...
    inserts.push({
      ...
      description: `Synkad från schema ${booking.id} [cat:${category}]`,
